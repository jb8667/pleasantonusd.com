...
//It will set the element on the window with respect to the top of the body
...
//It will set the Tab position on the window
function tabPos() {
    if (!isMobile) {
...
//This function helps the Tab functionality
function initTab() {
    ...
    link.href = window.url + "/Lets-Talk/css/LtTabLayout.css?k=65";
    link.rel = "Stylesheet";
...
//Check if the chatbot functionality is hidden
...
//It will check the events of the document when we click on the tab 
function documentClick(e) {
    ...
    //Issue No.  140468 : Triage : Change LT! Tab Logo HTML as per 508 Compliance

    if (isMobile) {
        if (S.className == 'previewDivMobile' || S.parentNode.id == 'previewDivMobile' || S.className == 'ltAssitantMenu' || S.className == 'ltAssistantMenuLogo' || S.className == 'ltAssistantMenuTextWrapper' || S.className == 'ltTabMenu' || S.className == 'ltTabMenuLogo' || S.className == 'ltTabMenuTextWrapper') {
        }
        else {
            if (isChatbot == true) {
...
//It will set the tab if it is loaded for the first time
...
function LTConfig(data) {
    ...
    if (isMobile) {
        ...
        if (MainCorpNoEncry == "WF7R4NLT") {
            var ChatBotMenuText = 'Send us a message and we will get back to you within a timely manner.';
        }
        else {
            var ChatBotMenuText = 'Send us a message and we will get back to you within ' + (data.PassDueDays > 1 ? data.PassDueDays + ' business days' : '1 business day') + '';
        }
        var ltTabMenuText = '<div class="ltTabMenuTextWrapper"><p class="title">Contact our Team</p><p class="subTitle">' + ChatBotMenuText + '</p></div>';
        ...
        if (data.TabCorpNo == "31834") {
            ...
            if (data.MobileIcon == 'mobileIcon1' || data.MobileIcon == 'mobileIcon2') {
                node.style.height = Math.ceil(data.MobileIconSize * 50 / 80) + 'px';
                textee.style.height = Math.ceil(data.MobileIconSize * 50 / 80) + 'px';
                textAnchor.style.height = Math.ceil(data.MobileIconSize * 50 / 80) + 'px';
                if ((isiPad.toString()).indexOf('iPad') != -1) {
        ...
        textAnchor.setAttribute('aria-label', data.ProductName + " Tab")
        ...
        loaderImgSRC.setAttribute('width', '419px');
        loaderImgSRC.setAttribute('height', '575px');
        ...
        if (isChatbot) {
            ...
            var textColor = '';
            var ChatDownArrow = 'ChatDownArrow.svg';
            var LTChatTabLogo = 'LTChatTabLogo.svg';
            var labeldark = '';
            if (data.TabColor == '#bb202e') {
                ...
                textColor = '#303030';
                ChatDownArrow = 'ChatDownArrowBlack.svg';
                LTChatTabLogo = 'LTChatTabLogoBlack.svg';
                labeldark = ' black';
            }
...
//Align the Tab bubble when using in iPad
...
//This function will take the user back to chat window
...
//Open the window menu which allow to change language and switch to LT tab
...
//This function will change the the language of the chatbot
...
//It will hide language menu
function hideLangMenu() {
    document.getElementById('ChatSwitchBot').style.display = 'none';
    document.getElementById('SwitchBackground').style.display = 'none';
}
//This function is used for removing unwanted parameters from URL string
...
//Close the preview tab of chat box
...
//Open the chat box with chatbot
...
//Open the window box to select language
...
//Hide the window to select the language if it is clicked outside the window box
...
//Change the language of the chat bot as per the selection of language by the user
function changeCombinedChatbotLanguage(langId) {
    ...
    //If the language is English then this statement will be executed
    if (languageId == 1) {
    ...
    //If the language is Spanish then this statement will be executed
